...
public class Database {
   ...
   static Logger log = Logger.getLogger(Database.class);
    ...
    public Database(String db, String driver) throws IOException, ClassNotFoundException, SQLException {
        ...
        Class.forName(getCfgValue(driver));   // загрузка нужного класса по названию (вместо импорта как сверху)
                
        // Create a connection to the database
        String user_name=getCfgValue((db + "_USER"));
        String user_pass=getCfgValue((db + "_PASSWORD"));

        connection= DriverManager.getConnection(url,user_name,user_pass);  //подключение к базе
    ...
    /**
     * Metod for Update, Insert and Delete
     * @param query
     * @return
     * @throws SQLException
     */
    public int changeTable (String query) throws SQLException {
        ...
        int effectedRows = statement.executeUpdate(query); //executeUpdate  - вернёт кол-во проапдейченных дат
                statement.close();
        return  effectedRows;
    }
    /*
